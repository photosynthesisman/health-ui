  ...
  <BaseBody>
    ...
    <section>
      <div class="action-buttons-container">
        <button class="action-button" @click="handleNewAddClick">
          ...
          <span class="action-button-text">서류없이 청구</span>
          <span class="action-button-text">가능 병원 확인</span>
        </button>

        <button class="action-button" @click="handleEditItemClick">
          <div class="action-button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
              <path
                fill-rule="evenodd"
          ...
          <span class="action-button-text">병원 서류 발급</span>
          <span class="action-button-text">가능 병원 확인</span>
        </button>
      </div>
      <div class="single-action-section">
        <button class="full-width-action-button" :disabled="!isResidentIdValid" @click="handleProceedClick">
          MY 병원 찾기
        </button>
      </div>
    </section>
...
// --- Resident ID related reactive data ---
const residentIdFront = ref('')
const residentIdBack = ref('')

// Computed property to check if both resident ID fields are valid
const isResidentIdValid = computed(() => {
  // Check if front part is 6 digits long and contains only numbers
  const isFrontValid = /^\d{6}$/.test(residentIdFront.value)

  // Check if back part is 7 digits long and contains only numbers
  const isBackValid = /^\d{7}$/.test(residentIdBack.value)

  // Both parts must be valid for the entire ID to be valid
  return isFrontValid && isBackValid
})

// Watch for changes in the front part to auto-focus to the back part
watch(residentIdFront, newValue => {
  if (newValue.length === 6) {
    document.getElementById('resident-id-back')?.focus()
  }
  // Ensure only digits are kept
  residentIdFront.value = newValue.replace(/[^0-9]/g, '')
})

// Watch for changes in the back part to ensure only digits are kept
watch(residentIdBack, newValue => {
  residentIdBack.value = newValue.replace(/[^0-9]/g, '')
})

// --- Action button handlers (optional, for demonstration) ---
const handleNewAddClick = () => {
  console.log('서류없이 청구 가능 병원 확인 클릭')
  // Implement your logic for this button
}
...
const handleProceedClick = () => {
  if (isResidentIdValid.value) {
    console.log('다음 단계로 진행 (MY 병원 찾기) 클릭')
    console.log('주민등록번호 앞자리:', residentIdFront.value)
    console.log('주민등록번호 뒷자리:', residentIdBack.value)
    // Implement logic to find MY hospital
  } else {
    console.log('주민등록번호가 유효하지 않아 다음 단계로 진행할 수 없습니다.')
