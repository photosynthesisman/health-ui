<template>
  <BaseBody>
    <div>걷기왕 프라이빗 게임 참가자 프로필(걷기왕-걷기게임-참가자프로필)</div>
  </BaseBody>
  <BottomModal :is-visible="isShowTeamJoinStatusModal" v-bind="teamJoinStatusModal" @close="toggleTeamJoinStatusModal">
    <template #content>
      ...
      <FlexRowDiv class="gap-8 mt-18 mb-32">
        <ChallengeStepsBox title="총 걸음" steps="954,298" />
        <ChallengeStepsBox title="총 이동거리" steps="26.3km" />
      </FlexRowDiv>
      <!-- 날짜 별 걸음수 현황판 -->
      <ActivityChart />
      <!-- 걷기 통계 -->
      <ChallengeStepsWrap :custom-class="'column2'">
        <ChallengeStepsBox title="걸음/오늘" steps="954,2" />
        <ChallengeStepsBox title="이동거리/오늘" steps="2.47km" />
        <ChallengeStepsBox title="걸음/일평균" steps="954,2" />
        <ChallengeStepsBox title="이동거리/일평균" steps="1.35km" />
      </ChallengeStepsWrap>
