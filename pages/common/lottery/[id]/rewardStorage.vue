<template>
  <BaseBody page-title=" 보관함" :show-back-button="true">
    <LineTabs :tabs="lineTabs" :active-key="activeLineTab" @tab-change="onLineTabChange" />
    <!-- 리워드보관함 컨텐츠 -->
    <div v-if="activeLineTab === 'reward'">
      <TotalCountBtnType :count="1" :btn-text="'전체 사용 내역 보기'" class="mg-21y" />
      <FlexRowDiv class="flex-end">
        <Select
          style="width: fit-content"
          modal-title="타입별"
          :transparent="true"
          select-placeholder="타입별"
          :custom-opts="[
            { value: '티켓', label: '티켓' },
            { value: '참가권', label: '참가권' },
            { value: '아이템', label: '아이템' },
            { value: '상품 교환권', label: '상품 교환권' }
          ]"
        />
      </FlexRowDiv>
      <RewardItemWrap>
    ...
    <!-- 구매보관함 컨텐츠 -->
    <div v-else-if="activeLineTab === 'purchase'">
      <TotalCountBtnType :count="1" :btn-text="'전체 사용 내역 보기'" class="mg-21y" />
      <FlexRowDiv class="flex-end">
        <Select
          style="width: fit-content"
