<template>
  <div class="weekly-keyword">
    ...
    >
      ...
      >
        <div v-for="(item, itemIndex) in slider" :key="`item-${item.id}-${itemIndex}`" class="community-item">
          <HashtagWrap :hashtags="(item.hashtags || []).slice(0, 10)" />
        </div>
      </div>
    </div>
  </div>
...
.weekly-keyword {
  ...
  .weekly-keyword-swiper-container {
    position: relative;
    overflow: hidden;
    padding-left: 2rem;
    margin: 0 -2rem;
    .weekly-keyword-swiper {
      display: flex;
      gap: 0 0.8rem;
      cursor: grab;
      user-select: none;
