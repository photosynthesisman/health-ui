<template>
  <div class="main-tip">
    <h5 class="tit"><strong class="user-name">레몬시큼</strong>님 맞춤 건강 TIP</h5>
    <div
      class="main-tip-container"
      ref="containerRef"
    ...
    >
      <MainTipCard v-for="(item, index) in tipItems" :key="item.id || index" v-bind="item" @click="handleCardClick" />
    </div>
  </div>
</template>
...
import MainTipCard from '~/components/publishing/healthDebt/MainTipCard.vue'
...
const props = withDefaults(defineProps<Props>(), {
  items: () => [
    { id: 1, title: '반드시 알아야 하는 폐암의 3가지 신호 1편' },
    { id: 2, title: '담배는 폐에 얼마나 안 좋은가요? 1편' },
    { id: 3, title: '반드시 알아야 하는 폐암의 3가지 신호 1편' },
    { id: 4, title: '담배는 폐에 얼마나 안 좋은가요? 1편' },
    { id: 5, title: '반드시 알아야 하는 폐암의 3가지 신호 1편' },
    { id: 6, title: '담배는 폐에 얼마나 안 좋은가요? 1편' }
  ],
  cardWidth: 20, // 20rem
  cardGap: 1.2 // 1.2rem
})
...
// 동적 데이터
const tipItems = computed(() => props.items)
const containerRef = ref<HTMLElement>()

// 이벤트 정의
const emit = defineEmits<{
  cardClick: [item: TipItem]
}>()
...
const getMaxScrollDistance = () => {
  ...
  const totalItems = tipItems.value.length
...
// 카드 클릭 핸들러
const handleCardClick = (item: TipItem) => {
...
const scrollToIndex = (index: number) => {
  if (index < 0 || index >= tipItems.value.length) return
